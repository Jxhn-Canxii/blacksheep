...
import { HiHome, HiChatBubbleLeftRight, HiUsers, HiHashtag, HiOutlineUser } from "react-icons/hi2";
...
import RecommendedSignals from "@/components/feed/RecommendedSignals";
...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const [recentGroups, setRecentGroups] = useState<Group[]>(initialRecentGroups);
  ...
  useEffect(() => {
    ...
    const fetchRecentGroups = async () => {
      ...
      if (error) {
        ...
        setRecentGroups(data.map((item: { groups: Group }) => item.groups).filter(Boolean));
      }
