...
import { getEmotionColor, emotionColors, getEmotionEmoji } from "@/libs/emotionConfig";
import { toggleFollow as toggleFollowApi } from "@/services/api";
...
import { Vent, Reply } from "@/interfaces/types";
...
/**
 * Returns actual CSS color values for an emotion, derived from the marker hex in emotionColors.
 * Used for inline styles in Leaflet divIcons (Tailwind dynamic classes are purged at build time).
 */
const getEmotionCssColors = (emotion: string): { bg: string; border: string; shadow: string; text: string } => {
  const e = emotion?.toLowerCase() || 'neutral';
  const entry = emotionColors[e] || emotionColors.neutral;
  const hex = entry.marker;
  return {
    bg: `${hex}cc`,           // ~80% opacity background
    border: `${hex}80`,       // ~50% opacity border
    shadow: `${hex}b3`,       // ~70% opacity glow
    text: '#ffffff',
  };
};
...
const Map: React.FC<MapProps> = ({ view = "public" }) => {
  ...
  const { data: vents, error, mutate: mutateVents } = useSWR<Vent[]>(
    ...
    async () => {
      ...
      if (view === "private" && user) {
        const following = await FollowsService.getFollowing(user.id);
        query = query.in('user_id', [user.id, ...following]);
      }
  ...
  const handleToggleFollow = useCallback(async (targetUserId: string) => {
    ...
    // Optimistic UI update; follow state will also be revalidated via the effect.
    setFollowingIds((prev) => {
      if (isCurrentlyFollowing) return prev.filter((id) => id !== targetUserId);
      return prev.includes(targetUserId) ? prev : [...prev, targetUserId];
    });
  }, [supabase, user, followingIds]);
  ...
  const createBubbleIcon = useCallback((emotion: string, replyCount: number, intensity?: number) => {
    const css = getEmotionCssColors(emotion);
    const emoji = getEmotionEmoji(emotion);
    return L.divIcon({
      className: 'custom-bubble-icon',
      html: `
        <div class="relative group">
           <div style="width:48px;height:48px;border-radius:1.5rem;background:${css.bg};border:2px solid ${css.border};box-shadow:0 0 20px ${css.shadow};display:flex;align-items:center;justify-content:center;animation:pulse 2s cubic-bezier(0.4,0,0.6,1) infinite;">
              <div style="display:flex;flex-direction:column;align-items:center;">
                <span style="font-size:14px;font-weight:900;color:${css.text};line-height:1;">${emoji}</span>
                ${intensity ? `<span style="font-size:7px;font-weight:700;color:${css.text};opacity:0.7;margin-top:2px;">Lvl ${intensity}</span>` : ''}
              </div>
           </div>
  ...
  const getVentIcon = (vent: Vent) => {
    const css = getEmotionCssColors(vent.emotion);
    const emoji = getEmotionEmoji(vent.emotion);
    return L.divIcon({
      html: `<div style="position:relative;display:flex;align-items:center;justify-content:center;width:32px;height:32px;border-radius:50%;border:2px solid ${css.border};background:${css.bg};box-shadow:0 4px 12px ${css.shadow};"><span style="font-size:18px;">${emoji}</span><div style="position:absolute;bottom:-4px;width:8px;height:8px;background:white;border-radius:50%;border:2px solid ${css.border};"></div></div>`,
  ...
  return (
    <div className="h-full w-full relative flex flex-col">
      ...
      >
        ...
        {vents?.map((vent: Vent, index: number) => {
          ...
          const overlaps = vents.filter((v: Vent, i: number) => i !== index &&
              ...
              <Popup className="premium-popup">
                <div className="p-6 bg-neutral-900 border border-white/5 text-white rounded-[2.5rem] min-w-[280px] shadow-3xl space-y-6 relative overflow-hidden">
                    ...
                    <div className="flex items-center justify-between border-b border-white/5 pb-4 relative z-10">
                        <div className="flex flex-col">
                            <div className="flex items-center gap-x-2">
                                ...
                                {vent.emotion && <EmotionBadge emotion={vent.emotion} />}
                                {user && !isOwn && view !== "public" && (
                                    <button 
                                        onClick={() => handleToggleFollow(vent.user_id)}
                                        className={twMerge(
                    ...
                    {view === "private" && vent.replies && vent.replies.length > 0 && (() => {
                        ...
                        const latestReply = [...vent.replies].sort((a: Reply, b: Reply) => 
