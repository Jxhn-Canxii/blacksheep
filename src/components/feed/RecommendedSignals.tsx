...
const RecommendedSignals = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Fetch following ids via API
      const following = await apiGet<string[]>('/api/follows', {
        params: { follower_id: currentUser.id },
      });
      setFollowingIds(following);

      // Fetch recommended profiles via service → /api/profiles
      const profiles = await fetchProfiles({
        exclude_id: currentUser.id,
        exclude_following: following.join(','),
        limit: 10,
      });
      ...
      <div className="flex flex-col gap-y-3 px-1">
        <AnimatePresence>
          {recommended.map((profile) => (
            ...
            >
              <Link href={`/profiles/${profile.id}`} className="flex items-center gap-x-3 flex-1 min-w-0">
                ...
                <div className="flex flex-col min-w-0">
                  ...
                  <div className="flex items-center gap-x-2">
                    <span className="text-[7px] font-bold text-neutral-600 uppercase tracking-widest whitespace-nowrap">{profile.followers_count} LINKS</span>
                    <span className="w-0.5 h-0.5 bg-neutral-800 rounded-full" />
                    <span className="text-[7px] font-bold text-neutral-600 uppercase tracking-widest whitespace-nowrap">{profile.vents_count} VENTS</span>
                  </div>
                </div>
              </Link>

              <button
                onClick={(e) => { e.preventDefault(); void toggleFollow(profile.id); }}
                className={twMerge(
