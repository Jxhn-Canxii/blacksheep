...
import { UserDetails, Vent } from "@/interfaces/types";
...
const SearchPage = () => {
    ...
    const [profiles, setProfiles] = useState<UserDetails[]>([]);
    ...
    useEffect(() => {
        const fetchFollowData = async () => {
            if (!user) return;
            const followStats = await FollowsService.getFollowStats(user.id);
            setFollowingIds(followStats.following);
            setFollowerIds(followStats.followers);
        };
        fetchFollowData();
    }, [user]);
    ...
    const toggleFollow = async (targetId: string) => {
        ...
        const result = await FollowsService.toggleFollow(user.id, targetId);
    ...
    useEffect(() => {
        ...
        const fetchResults = async () => {
            ...
            if (!ventsRes.error && ventsRes.data) {
                setVents(ventsRes.data as Vent[]);
            }
            if (!profilesRes.error && profilesRes.data) {
                setProfiles(profilesRes.data as UserDetails[]);
            }
