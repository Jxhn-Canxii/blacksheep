...
interface GroupCardProps {
  ...
  i: number;
  user: User | null;
  joinGroup: (groupId: string, isPrivate: boolean) => void;
  loadingCircles: Set<string>;
}

const GroupCard = memo(({ group, i, user, joinGroup, loadingCircles }: GroupCardProps) => {
  const isMember = group.group_members?.some((m: GroupMember) => m.user_id === user?.id);
  const isPending = group.group_members?.find((m: GroupMember) => m.user_id === user?.id)?.status === 'pending';
  ...
  return (
    ...
    >
        ...
        <div className="flex-1 bg-gradient-to-br from-neutral-900/90 via-neutral-900/70 to-emerald-500/5 backdrop-blur-3xl p-8 rounded-[2.5rem] border border-white/10 group-hover:border-emerald-500/50 transition-all duration-500 shadow-2xl relative overflow-hidden flex flex-col justify-between cursor-default">
            ...
            <div className="relative z-10">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <div className="flex -space-x-2.5">
                        {group.group_members?.slice(0, 4).map((m: GroupMember, idx: number) => (
            ...
            <div className="flex items-center justify-end mt-12 relative z-10">
...
const GroupsPage = () => {
  ...
  const { data: groups, error, mutate } = useSWR<Group[]>(
    ...
    async () => {
      ...
      return (data as Group[]).map(group => ({
        ...group,
        _count: {
  ...
  const userGroups = useMemo(() => {
    ...
    return groups.filter(g => g.group_members?.some((m: GroupMember) => m.user_id === user.id));
  }, [groups, user]);
