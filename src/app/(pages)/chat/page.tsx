...
const GlobalChatPage = () => {
  ...
  const loadMessages = async (offset = 0) => {
    ...
    try {
      const result = await fetchMessages(offset, MESSAGES_PER_PAGE);
      if (offset === 0) {
  ...
  useEffect(() => {
    ...
    // Realtime subscription stays on the Supabase client — it's a push channel, not a data fetch
  ...
  useEffect(() => { void loadFollowing(); }, [user]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
    if (e.currentTarget.scrollTop === 0 && hasMore && !loadingMore) {
      void loadMessages(messages.length);
    }
  };
    ...
    <div className="bg-neutral-900 rounded-[3rem] h-full w-full flex flex-col overflow-hidden relative border border-white/5 shadow-2xl">
        ...
        {loading ? (
          ...
          <AnimatePresence initial={false}>
            {messages.map((message) => {
              ...
              const anonymousName = message.user_id
                ? `Anonymous-${message.user_id.slice(-4)}`
                : "Anonymous-0000";
                      ...
                      {!isOwn && (
                        <div className="flex items-center justify-between mb-1">
                          ...
                          {user && user.id !== message.user_id && (
                            <button
                              onClick={() => void handleToggleFollow(message.user_id)}
                              className={twMerge(
      ...
      <div className="p-8 bg-neutral-900/50 backdrop-blur-3xl border-t border-white/5 relative z-10">
        <form
          ...
          className="flex gap-x-4 bg-neutral-800/80 p-2 rounded-full border border-white/5 focus-within:border-emerald-500/30 transition-all duration-300 shadow-2xl"
        >
          <input
            type="text"
            value={newMessage}
